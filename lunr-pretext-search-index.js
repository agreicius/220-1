...
},
{
  ...
  "body": " Curve sketching II     Bring all our calculus tools to bear on the analysis of a real-valued function . In more detail: determine the domain, the -intercept and any -intercepts; compute the limit of the function at endpoints of the domain and determine whether there are horizontal asymptotes; determine whether there are vertical asymptotes; find and classify all critical points of ; determine the intervals of monotonicity; determine intervals of constant concavity; identify any inflection points.    Express the results of our analysis of a function's behavior in the form of a detailed sketch.      Curve sketch: trig   Provide a graph of that includes all the details listed in to the best of your abilities .    Although the domain of is , observe that since for all , we can obtain a graph of by producing a careful sketch on the interval and then copying and pasting this graph over all other intervals . This means for the moment our analysis will be restricted to just the interval . The -intercept, as always, is . To find the -intercepts in we solve: . Next we compute . From the factored form of , we see solve . With a little work, choosing our test points carefully, we produce the following sign diagram for on the interval .   Sign diagram for derivative of f    The diagram clearly indicates the intervals of monotonicity within , and indicates that attains in alternating manner local maximum\/minimum values at these inputs.  From this information, and the table of values we can obtain the following sketch of on .   Graph of f on restricted domain    As mentioned above, using the periodicity of , we get a full sketch of on by cutting and pasting .   Graph of f over all reals    Note that as a result of the periodicity of , the absolute maximum and minimum values of on the domain are equal to the absolute maximum and minimum values of on the interval . From our table of values above, which includes evaluations at all critical points in , along with , we see that is the absolute maximum value of on , and is the absolute minimum value.  What about the concavity of and inflection points? The second derivative is somewhat complicated, making a precise sign diagram for difficult to produce. Our graph of above seems to suggest four inflection points within the interval .   Graph with inflection points    Can we at least verify this algebraically? Yes, we can!  Let , so that . We can solve for using the quadratic equation: . Thus we have a potential inflection point at points satisfying . Each of the equations above has two solutions within , and these correspond to the four inflection points we see in our graph of over .     Curve sketching: rational function   Provide a graph of that includes all the details listed in .    The domain of is , and is both the -intercept and the sole -intercept.  Since is continuous at , we have . Next we compute . Thus is a horizontal asymptote of .  The only candidate for a vertical asymptote of is . We compute . In particular, we see that is a vertical asymptote of the graph of .  Moving on, after some careful simplification, we see that for all . Note that both and are both undefined at . It follows that is the only critical point of . We now compute the sign diagram of .   Sign diagram for derivative of f    Not in particular that is decreasing on its entire domain.  To produce the sign diagram of we first solve . Since only lies in the domain of , we obtain the following sign diagram for .   Sign diagram for second derivative of f    Putting it all together, we get a graph like the following.   Graph of f    Observe how easy it would be to overlook the inflection point (the sole point plotted above in red) without our careful analysis.    "
},
{
...
},
{
  ...
  "body": " Curve sketch: trig   Provide a graph of that includes all the details listed in to the best of your abilities .    Although the domain of is , observe that since for all , we can obtain a graph of by producing a careful sketch on the interval and then copying and pasting this graph over all other intervals . This means for the moment our analysis will be restricted to just the interval . The -intercept, as always, is . To find the -intercepts in we solve: . Next we compute . From the factored form of , we see solve . With a little work, choosing our test points carefully, we produce the following sign diagram for on the interval .   Sign diagram for derivative of f    The diagram clearly indicates the intervals of monotonicity within , and indicates that attains in alternating manner local maximum\/minimum values at these inputs.  From this information, and the table of values we can obtain the following sketch of on .   Graph of f on restricted domain    As mentioned above, using the periodicity of , we get a full sketch of on by cutting and pasting .   Graph of f over all reals    Note that as a result of the periodicity of , the absolute maximum and minimum values of on the domain are equal to the absolute maximum and minimum values of on the interval . From our table of values above, which includes evaluations at all critical points in , along with , we see that is the absolute maximum value of on , and is the absolute minimum value.  What about the concavity of and inflection points? The second derivative is somewhat complicated, making a precise sign diagram for difficult to produce. Our graph of above seems to suggest four inflection points within the interval .   Graph with inflection points    Can we at least verify this algebraically? Yes, we can!  Let , so that . We can solve for using the quadratic equation: . Thus we have a potential inflection point at points satisfying . Each of the equations above has two solutions within , and these correspond to the four inflection points we see in our graph of over .   "
},
{
  "id": "eg_curve_radical",
  "level": "2",
